...
class MyMatplotlibFigure(FigureCanvasQTAgg):
    ...
    #saturation mat
    def mat_plot_drow_axes_s(self, t, s1):
        """
        用清除画布刷新的方法绘图
        :return:
        """
...
class MyWindow(QtWidgets.QMainWindow, gui630.Ui_MainWindow):
    ...
    def b_atp_set(self):
        p = self.b_atbox_p.value()  # 获取a框设置的值
    ...
    def b_manu(self):
        ...
        # address2 = self.laser2Box_gpib.currentText()
    ...
    def s_atp_set(self):
        p = self.s_atbox_p.value()  # 获取a框设置的值
    ...
    def s_manu(self):
        ...
        try:
            ...
            if self.checkBox.isChecked():
                ...
                y2 = []
                self.statusbar.showMessage(
                    'Photocurrent (mA)=%s,   RF Power (dBm)=%s' % (abs(round(sm_output2, 3)), pm_output))
                self.statusbar.setToolTipDuration(5000)
                ...
                self.canvas2.figs.supylabel('RF Power (dBm)', fontsize=12, fontfamily='Calibri')
                self.canvas2.figs.subplots_adjust(left=0.14, right=0.9, bottom=0.14)
                self.canvas2.mat_plot_drow_axes_s(x1, y1)
    ...
    def s_clear(self):
        ...
        self.ys.clear()
        self.ys1.clear()
        self.canvas2.mat_plot_drow_axes_s(self.xs, self.ys)

    def s_stop(self):
        address = self.s_atbox_gpib.currentText()  # 衰减器地址
        p = self.s_atbox_p.value()
        # p = p - 10
        AT = N7761A.KeysightN7761A("GPIB0::" + str(address) + "::INSTR")
        AT.at_factor = 10
        self.s_atbox_p.setValue(10)
